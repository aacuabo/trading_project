...
import yaml
from yaml.loader import SafeLoader # Required for stauth load_config

# --- User Authentication ---
# Load authentication configuration from secrets.toml
# We need to load the credentials structure correctly from secrets
# Streamlit secrets are accessed as a dictionary, so construct the credentials
# dictionary in the format expected by streamlit-authenticator
# from the secrets structure.
...
def get_sqlalchemy_engine():
    ...
    try:
        # Read database credentials from secrets.toml (already loaded for auth)
...
# --- LOAD DATA ---
# @st.cache_data # Consider caching this function if available dates don't change often
...
if authentication_status == False:
    ...
    authenticator.logout('Logout', 'main') # Add a logout button
    ...
    if not data.empty:
        ...
        if "Prices" in data.columns and not data["Prices"].empty:
            # Ensure Prices is numeric before calculation
            if pd.api.types.is_numeric_dtype(data["Prices"]):
                 ...
                 col1.warning("Prices column is not numeric.")
                 col2.warning("Prices column is not numeric.")
        else:
        ...
        # st.subheader("Hourly Summary") # Already have subheader above dataframe
        # # Display the fetched data as a table
        # st.dataframe(data)
